...
class LSH:
    ...
    def create_bucket_hash(self, band_index: int, band_signature: list):
        """
        ...
        """
        hash_code = band_index  # initialize hash_code variable with the band index
        for val in band_signature:
            # Avoid overflow and ensure a 32-bit hash
            hash_code = (hash_code * 31 + val) % (2**32-1) # (2**32-1) it is an example of a large Mersenne prime number
        
        # Ensure the hash is positive and limit to 6 digits
        bucket_code = str(hash_code % 1000000).zfill(6)
        
        # Format as LSH-NNNNNN
        return f"LSH-{bucket_code[:6]}"
    ...
    def build_buckets(self, signatures: dict):
        ...
        for user_id, signature in signatures.items():
            ...
            for band_index in range(self.num_bands):
                ...
                # Extract band signature for hashing
                band_signature = signature[start:end_index]
                
                # Create bucket key using custom hash function
                bucket_key = self.create_bucket_hash(band_index, band_signature)
    ...
    def query(self, query_signature: list, query_user_id: int):
        ...
        for band_index in range(self.num_bands):
            ...
            # Calculate start and end indices for the current band
            start_index = band_index * self.rows_per_band
            end_index = start_index + self.rows_per_band
            
            # Extract band signature 
            band_signature = query_signature[start_index:end_index]
