...
class MinHash:
    ...
    def create_signature(self, movie_set):
        ...
        # Convert the set of movie IDs into a NumPy array for vectorized operations
        # allowing faster computation than Python lists.
        movie_ids = np.array(list(movie_set))  
        
        # Calculate all hash values 
        for i in range(self.n_hash_functions):
            hash_values = self.hash_function(movie_ids, self.a[i], self.b[i],i)
            # Update the signature by taking the minimum hash value for the current hash function
